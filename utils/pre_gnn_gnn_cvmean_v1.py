...
class Head(nn.Module):
    def __init__(self, hid=128, dropout1=0.28, dropout2=0.18):
        super().__init__()
...
# helpers
# =========================================================
def prepare_master_df_graph_only(csv_path):
    ...
    df = df[["SMILES", "logBCF"]].dropna(subset=["SMILES", "logBCF"]).copy()
...
class GraphOnlyDataset(Dataset):
    def __init__(self, df, y_mean=None, y_std=None, fit=False):
        ...
        self.smiles = [self.smiles_raw[i] for i in keep]
        self.row_ids = [self.row_ids_raw[i] for i in keep]
        self.y = self.y_raw[keep].astype(np.float32)

        if fit:
            self.y_mean = float(np.mean(self.y))
            self.y_std = float(np.std(self.y)) if float(np.std(self.y)) > 1e-8 else 1.0
        else:
            self.y_mean = float(y_mean)
            self.y_std = float(y_std) if float(y_std) > 1e-8 else 1.0

        self.y_scaled = ((self.y - self.y_mean) / self.y_std).astype(np.float32)

    def __len__(self):
        return len(self.G)

    def __getitem__(self, idx):
        return self.G[idx], torch.tensor(self.y_scaled[idx], dtype=torch.float32), self.smiles[idx], self.row_ids[idx]


def build_dataset_fit(df):
    ds = GraphOnlyDataset(df, fit=True)
    return ds, ds.y_mean, ds.y_std


def build_dataset_eval(df, y_mean, y_std):
    return GraphOnlyDataset(df, y_mean=y_mean, y_std=y_std, fit=False)


def collate(batch):
    gs, y, smi, rid = zip(*batch)
    return dgl.batch(gs), torch.stack(y), list(smi), list(rid)
...
def predict(enc, head, loader, inv, device, use_ema=False, ema_enc=None, ema_head=None):
    ...
    for g, y, smiles, row_ids in loader:
        smiles_all += smiles
        row_ids_all += list(row_ids)
        g = g.to(device)
        p = head(enc(g)).detach().cpu()
...
def load_state_for_eval(state_obj, args, device):
...
def train_cv_fold_curve(train_df, valid_df, feats, device, args, seed, verbose_prefix=""):
    ...
    _, y_mean, y_std = build_dataset_fit(train_df)
    tr_ds = build_dataset_eval(train_df, y_mean, y_std)
    va_ds = build_dataset_eval(valid_df, y_mean, y_std)
    ...
    enc, head, meta = init_models(args, device)
...
# =========================================================
def main():
    ap = argparse.ArgumentParser(description="Pre-GNN / GNN graph-only with CV-curve epoch selection")
    ap.add_argument("--csv", required=True)
    ap.add_argument("--features", required=False, default=None)
    ap.add_argument("--outer-split-file", required=True)
    ap.add_argument("--outdir", default="pregate_dagnn_cvmean_results")

    ap.add_argument("--no-tl", action="store_true", help="Use GNN; default is Pre-GNN")
    ...
    feats = []
    master_df = prepare_master_df_graph_only(args.csv)
    ...
    model_name = "GNN-CVMean" if args.no_tl else "Pre-GNN-CVMean"
    ...
    for split_id, seed in split_pairs:
        ...
        train_ds = build_dataset_eval(train_df, y_mean, y_std)
        val_ds = build_dataset_eval(val_df, y_mean, y_std)
        ...
        enc, head = load_state_for_eval(final_state, args, device)
